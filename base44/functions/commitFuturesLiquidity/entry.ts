...
Deno.serve(async (req) => {
  try {
    ...
    if (existingOffers.length > 0) {
      ...
      // CRITICAL FIX: Find existing LP UserBet for this offer and reuse it
      // This prevents multiple UserBets for the same position, ensuring correct match % calculation
      const existingUserBets = await serviceRole.entities.UserBet.filter({
        offer_id: existingOffer.id,
        role: 'lp',
      });
      if (existingUserBets.length > 0) {
        existingUserBetId = existingUserBets[0].id;
      }
    } else {
    ...
    // CRITICAL: If reusing existing offer, update the existing UserBet instead of creating duplicate
    if (existingUserBetId) {
      // Reuse existing UserBet - add to its deposited and unmatched amounts
      const existingUserBet = await serviceRole.entities.UserBet.get(existingUserBetId);
      ...
      // Delete the newly created duplicate UserBet
      await serviceRole.entities.UserBet.delete(commit_data.userBetId);
    } else {
      // First time LP for this outcome - use the newly created UserBet
